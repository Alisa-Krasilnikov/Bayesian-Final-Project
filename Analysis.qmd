...
3.  Dataset: https://www.kaggle.com/datasets/kritirathi/indian-food-dataset-with

4.  Log Reg Assumptions: https://www.bayesrulesbook.com/chapter-13#eq:log-model-ch13
...
Additionally, we fit a linear model, specifically a generalized linear model (glm) instead of a linear model(lm) because our variables are not continuous and are instead categorical.

```{r}
glm(dum_diet ~ flavor_profile, data = indfood, family = "binomial") 
```

The extreme size of our coefficients(spicy being -18) confirms what we would expect with the quasi-complete separation of our data.
